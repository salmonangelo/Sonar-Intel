...
export const GisMappingPage: React.FC<GisMappingPageProps> = ({
  ...
  const totalSourceCount = sourceContacts.length;
  const highPriorityCount = sourceContacts.filter(c => c.priority === 'HIGH' && c.review_status !== 'CONFIRMED').length;
  const mediumPriorityCount = sourceContacts.filter(c => c.priority === 'MEDIUM' && c.review_status !== 'CONFIRMED').length;
  const lowPriorityCount = sourceContacts.filter(c => c.priority === 'LOW' || c.review_status === 'CONFIRMED').length;

  const filteredContacts = sourceContacts.filter(c => {
    // 1. Header Filter Pills
    if (filterMode === 'high' && (c.priority !== 'HIGH' || c.review_status === 'CONFIRMED')) return false;
    ...
    if (!priorityFilters.all) {
      if (c.priority === 'HIGH' && c.review_status !== 'CONFIRMED' && !priorityFilters.high) return false;
      if (c.priority === 'MEDIUM' && c.review_status !== 'CONFIRMED' && !priorityFilters.medium) return false;
      if ((c.priority === 'LOW' || c.review_status === 'CONFIRMED') && !priorityFilters.low) return false;
    }
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <label className="flex items-center gap-2 p-1.5 rounded-lg bg-[#f8fafc] border border-[#e2e8f0] cursor-pointer hover:bg-slate-100 transition-colors">
                    ...
                    />
                    <span className="font-semibold text-[#0f172a]">All ({totalSourceCount})</span>
                  </label>
                  <label className="flex items-center gap-2 p-1.5 rounded-lg bg-[#f8fafc] border border-[#e2e8f0] cursor-pointer hover:bg-slate-100 transition-colors">
                    <input
                      type="checkbox"
                    ...
                    <span className="font-semibold text-rose-700 flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-rose-500" /> High ({highPriorityCount})
                    </span>
                  </label>
                  <label className="flex items-center gap-2 p-1.5 rounded-lg bg-[#f8fafc] border border-[#e2e8f0] cursor-pointer hover:bg-slate-100 transition-colors">
                    <input
                    ...
                    <span className="font-semibold text-amber-700 flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-500" /> Med ({mediumPriorityCount})
                    </span>
                  </label>
                  <label className="flex items-center gap-2 p-1.5 rounded-lg bg-[#f8fafc] border border-[#e2e8f0] cursor-pointer hover:bg-slate-100 transition-colors">
                    <input
                    ...
                    <span className="font-semibold text-yellow-700 flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-yellow-400" /> Low ({lowPriorityCount})
                    </span>
                  </label>
                </div>
              </div>
