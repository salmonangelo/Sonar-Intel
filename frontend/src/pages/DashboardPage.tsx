...
export const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  const mediumPriorityCount = hasContacts 
    ? contacts.filter(c => c.priority === 'MEDIUM' && c.review_status !== 'CONFIRMED').length 
    : defaultTriageQueue.filter(c => c.priority === 'MEDIUM').length;
  const lowPriorityCount = hasContacts 
    ? contacts.filter(c => c.priority === 'LOW' || c.review_status === 'CONFIRMED').length 
    : defaultTriageQueue.filter(c => c.priority === 'LOW').length;
  const totalSurveys = survey ? 1 : (stats?.total_surveys ? Math.min(stats.total_surveys, 1) : 1);

  const triageList = hasContacts
    ? contacts.map((c, idx) => ({
  ...
  const metricsCards = [
    ...
    },
    {
      ...
      subtext: totalDetections > 0 ? `${Math.round((confirmedCount / totalDetections) * 100)}% Verified` : '0% Verified',
      icon: CheckCircle2,
    ...
    <div className="p-6 lg:p-8 max-w-[1700px] mx-auto space-y-6 lg:space-y-8 font-sans text-[#0f172a]">
          ...
          <div className="flex items-center gap-3 bg-[#f8fafc] border border-[#e2e8f0] rounded-xl px-4 py-2.5 text-xs shadow-xs">
            ...
            <div>
              ...
              <div className="font-mono font-bold text-[#0f172a] truncate max-w-[260px] text-xs">
                {survey?.filename || 'viator_04_test_wreck.png'}
              </div>
            </div>
          </div>
          ...
          <div className="p-4 px-6 border-b border-[#e2e8f0] bg-[#f8fafc] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
            ...
            <div className="flex flex-wrap items-center gap-2 font-semibold text-[11px]">
              ...
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white border border-[#e2e8f0] shadow-2xs">
                ...
                <span className="text-[#0f172a]">Low / Confirmed ({lowPriorityCount})</span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-6 flex flex-col justify-between">
          ...
          <div className="bg-white rounded-[24px] border border-[#e2e8f0] p-6 shadow-soft space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${totalDetections > 0 ? (highPriorityCount / totalDetections) * 100 : 0}%` }} 
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${totalDetections > 0 ? (mediumPriorityCount / totalDetections) * 100 : 0}%` }} 
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${totalDetections > 0 ? (confirmedCount / totalDetections) * 100 : 0}%` }}
                  />
                </div>
              </div>
            </div>
            ...
            <div className={`p-3 rounded-xl border flex items-center justify-between text-xs font-semibold ${
              ...
              <span>
                {highPriorityCount > 0 
                  ? `${highPriorityCount} High Priority Target${highPriorityCount === 1 ? '' : 's'}` 
                  : confirmedCount > 0 
                  ? `${confirmedCount} Confirmed Debris Target${confirmedCount === 1 ? '' : 's'}`
                  : '0 High Priority Targets (All Clear)'}
              </span>
            </div>
          </div>
      ...
      <section className="space-y-4">
        ...
        <div className="flex justify-end pt-1">
          ...
          >
            <span>Review {highPriorityCount} High Priority Target{highPriorityCount === 1 ? '' : 's'}</span>
